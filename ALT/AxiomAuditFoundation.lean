...
Provenance: Paper I appendix ("Every theorem is `#print axioms`
...
triple, with **no named axioms anywhere**. The parallel `𝗜𝚺₁` witnesses that once
...
-- `ISigma1_delta1Definable` — are retired; their guards are removed. The axiom-clean
...
`ISigma1_delta1Definable` is retired, so nothing here carries a named axiom. -/
